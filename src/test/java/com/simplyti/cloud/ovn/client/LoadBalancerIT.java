...
public class LoadBalancerIT {
	...
	public void createLoadBalancer() throws InterruptedException{
		...
		Vip vip = new Vip("192.168.1.9", 80, Protocol.TCP);
		Collection<Address> ips = Collections.singleton(new Address("10.0.0.1", 80));
		Future<UUID> createResult = client.createLoadBalancer("test",Collections.singleton("testLs"),vip,ips,TEST_ENTRY_ID).await();
		assertThat(createResult.getNow(),notNullValue());
		
		Future<LoadBalancer> futureLoadBalancer = client.getLoadBalancer("test",Protocol.TCP).await();
		...
		assertThat(futureLoadBalancer.getNow().getName(),equalTo("test:tcp"));
	...
	public void createVipInExistingLoadBalancer() throws InterruptedException{
		...
		Future<UUID> createResult1 = client.createLoadBalancer("test",Collections.singleton("testLs"),new Vip("192.168.1.9", 80,Protocol.TCP),
		...
		Future<UUID> createResult2 = client.createLoadBalancer("test",Collections.singleton("testLs"),new Vip("192.168.1.9", 443,Protocol.TCP),
	...
	public void loadBalancerCreationWithSameDataShoulBeIdempotent() throws InterruptedException{
		...
		Vip vip = new Vip("192.168.1.9", 80, Protocol.TCP);
		UUID uuid = client.createLoadBalancer("test",Collections.singleton("testLs"),vip,Collections.singleton(new Address("10.0.0.1", 80)),TEST_ENTRY_ID).await().getNow();

		Future<UUID> result = client.createLoadBalancer("test",Collections.singleton("test"),vip,Collections.singleton(new Address("10.0.0.1", 80)),TEST_ENTRY_ID).await();
	...
	public void deleteLoadBalancerByName() throws InterruptedException{
		...
		Vip vip = new Vip("192.168.1.9", 80, Protocol.TCP);
		Collection<Address> ips = Collections.singleton(new Address("10.0.0.1", 80));
		client.createLoadBalancer("test",Collections.singleton("testLs"),vip,ips,TEST_ENTRY_ID).await();
		
		Future<Void> deleteResult = client.deleteLoadBalancer("test",Protocol.TCP).await();
		assertTrue(deleteResult.isSuccess());
		
		Future<LoadBalancer> result = client.getLoadBalancer("test",Protocol.TCP).await();
	...
	public void attachLoadBalancerToLogicalSwitch() throws InterruptedException{
		...
		Vip vip = new Vip("192.168.1.9", 80, Protocol.TCP);
		Collection<Address> ips = Collections.singleton(new Address("10.0.0.1", 80));
		Future<UUID> lb = client.createLoadBalancer("lb1",Collections.singleton("testLs"),vip,ips,TEST_ENTRY_ID).await();
		client.createLogicalSwitch("switch1",TEST_ENTRY_ID).await();
		
		Future<Void> result = client.attachLoadBalancerToSwitch("lb1",Protocol.TCP,"switch1").await();
		assertTrue(result.isSuccess());
	...
	public void dettachLoadBalancerToLogicalSwitch() throws InterruptedException{
		...
		client.attachLoadBalancerToSwitch("lb1",Protocol.TCP,"switch1").await();
	...
	public void deleteLoadBalancerVip() throws InterruptedException{
		...
		createResult = client.createLoadBalancer("test",
				Collections.singleton("testLs"),
				new Vip("192.168.1.9", 443, Protocol.TCP),
		...
		Future<Void> deleteResult = client.deleteLoadBalancerVip("test", new Vip("192.168.1.9", 80, Protocol.TCP)).await();
		assertTrue(deleteResult.isSuccess());
